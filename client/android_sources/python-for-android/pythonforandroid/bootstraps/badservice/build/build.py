...
def compile_dir(dfn):
    ...
    # import compileall
    # compileall.compile_dir(dfn, quiet=1)
...
def make_package(args):
    ...
    shutil.copytree(
        os.path.join('templates', 'drawable', 'drawable-xxhdpi'),
        os.path.join('res', 'drawable')
    )

    if args.icon:
        os.unlink(os.path.join('res', 'drawable', 'icon.png'))
        shutil.copy(
            args.icon or default_icon,
            os.path.join('res', 'drawable', 'icon.png')
        )
    ...
    render(
        'BroadcastLauncher.tmpl.java',
        os.path.join(
            'src',
            args.package.replace('.', os.path.sep),
            'BroadcastLauncher.java'
        ),
        args=args
    )
...
''')
    ...
    ap.add_argument('--sign', action='store_true') # Just ignore
